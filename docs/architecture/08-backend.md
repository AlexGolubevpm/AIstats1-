...
| --- | --- | --- | --- |
| `adspyglass:geo` | каждый час, :05 | вчера + сегодня | Гео-разрез → `FactRevenue` (`zoneId = null`) |
| `adspyglass:zones` | каждый час, :10 | вчера + сегодня | Зонный разрез → `FactRevenue` (`countryCode = 'ZZ'`) |
...
- **Лимиты.** `limiter` BullMQ на очередь AdSpyglass (значение после спайка, A7), Метрика — не больше 5 параллельных запросов.
