...
| A2 | Есть ли разрез по сетке (`network` / `broker` / `ad_network`)? | На нём держатся `v_network_geo`, алерт «инверсия waterfall», рекомендация флора, `get_network_matrix` | Блокер для этих фич. Смотреть UI-выгрузки ASG и спрашивать поддержку. До ответа — релиз без сеток |
...
| A7 | Лимиты ASG API | 40 сайтов × 3 разреза × 5 дней каждый час | Очередь с `limiter` в BullMQ |

**Шаг 0 плана — это спайк на полчаса:** curl-ом перебрать варианты `group_by`, сохранить ответы в `docs/asg-samples/`, закрыть A1–A7.
